    ...
    <section className="glass sky-card overflow-hidden rounded-3xl">
      <header className="flex items-center justify-between border-b border-vault-border px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold tracking-tight text-vault-text">Boarding Pass Monitor</h2>
            <p className="text-sm text-vault-muted">Autonomous resolution / live flight tracking</p>
          </div>
        </div>
        <span className="sky-mono flex items-center gap-2 text-xs text-vault-muted">
      ...
      <div className="sky-scanlines">
        {/* Column header row. */}
        <div className="sky-mono grid grid-cols-[1.1fr_1.4fr_1fr] gap-2 border-b border-vault-border/60 px-6 py-2.5 text-[10px] uppercase tracking-[0.18em] text-vault-muted sm:grid-cols-[0.8fr_1fr_1.6fr_1fr]">
          <span>Flight</span>
              ...
              return (
                <li
                  ...
                  className={`sky-mono grid grid-cols-[1.1fr_1.4fr_1fr] items-center gap-2 px-6 py-3.5 text-sm transition-colors sm:grid-cols-[0.8fr_1fr_1.6fr_1fr] ${
                    view.checking ? "bg-vault-accent-soft" : "hover:bg-vault-panel-2"
                  }`}
                >
                  {/* Flight + departure time. */}
                  <div className="min-w-0">
                    <p className="truncate font-bold tracking-widest text-vault-text">{p.flightCode}</p>
                  ...
                  {/* Coverage. */}
                  <p className="hidden text-vault-muted sm:block">
                  ...
                  {/* Payout result. */}
                  <p className="text-right text-xs font-semibold" style={{ color: settled ? "var(--sky-lime)" : "var(--muted)" }}>
