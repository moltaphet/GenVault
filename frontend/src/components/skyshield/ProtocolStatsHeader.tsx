  ...
  return (
    <div className="glass sky-card group relative overflow-hidden rounded-3xl p-5">
      ...
      <div className="flex items-center gap-3">
        <span
          className="inline-flex h-10 w-10 items-center justify-center rounded-2xl border border-vault-border"
          style={{ color: accent, boxShadow: `0 0 18px -6px ${accent}` }}
        >
          {icon}
        </span>
        <p className="text-xs font-medium uppercase tracking-[0.18em] text-vault-muted">{label}</p>
      </div>
      <p className="sky-mono sky-neon mt-4 text-3xl font-bold tracking-tight text-vault-text" style={{ color: accent }}>
        {value}
      </p>
      <p className="mt-1 text-xs text-vault-muted">{hint}</p>
    </div>
  );
}
