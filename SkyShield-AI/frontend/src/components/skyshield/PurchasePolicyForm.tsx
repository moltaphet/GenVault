  ...
  return (
    <section className="glass sky-card rounded-3xl p-6">
      <header className="mb-5 flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold tracking-tight text-sky-text">Insure a Flight</h2>
          <p className="text-sm text-sky-muted">Parametric delay coverage, priced by AI underwriting.</p>
        </div>
      </header>
      ...
      <div className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Flight Code">
            <input
              ...
              maxLength={8}
              className="sky-mono w-full rounded-xl border border-sky-border bg-sky-panel-2 px-4 py-2.5 uppercase tracking-widest text-sky-text outline-none transition-all duration-200 placeholder:text-sky-muted placeholder:tracking-normal focus:border-sky-cyan focus:ring-2 focus:ring-sky-cyan/30"
            />
          </Field>
          <Field label="Departure (local)">
            <input
              ...
              className="sky-mono w-full rounded-xl border border-sky-border bg-sky-panel-2 px-4 py-2.5 text-sky-text outline-none transition-all duration-200 focus:border-sky-cyan focus:ring-2 focus:ring-sky-cyan/30"
            />
          </Field>
        </div>
              ...
              return (
                <button
                  ...
                  className={`rounded-xl border px-3 py-3 text-left transition-all duration-200 ${
                    selected
                      ? "border-sky-cyan bg-sky-cyan-soft"
                      : "border-sky-border bg-sky-panel-2 hover:border-sky-border-strong"
                  }`}
                >
                  <p className="text-sm font-semibold text-sky-text">{p.label}</p>
                  <p className="sky-mono mt-1 text-xs text-sky-cyan">
                    {attoToToken(p.coverageAtto, 0)} {TOKEN_SYMBOL}
                  </p>
                  <p className="mt-1 text-[10px] leading-tight text-sky-muted">{p.tagline}</p>
                </button>
              );
            })}
          </div>
        ...
        <div className="sky-scanlines overflow-hidden rounded-2xl border border-sky-border bg-sky-panel-2 p-4">
          <div className="flex items-center justify-between">
            <span className="flex items-center gap-2 text-xs font-medium uppercase tracking-[0.18em] text-sky-muted">
              <Sparkles className="h-3.5 w-3.5 text-sky-cyan" /> Dynamic Premium Quote
            </span>
            {quote && (
              <span
                className="sky-mono rounded-full border px-2.5 py-0.5 text-[10px] font-semibold"
                style={{ color: "var(--magenta)", borderColor: "var(--border-strong)" }}
              >
          ...
          {quote ? (
            <div className="mt-3 flex items-end justify-between gap-4">
              <div>
                <p className="sky-mono sky-neon text-3xl font-bold text-sky-text">
                  {attoToToken(quote.premium, 2)}
                </p>
                <p className="text-xs text-sky-muted">{TOKEN_SYMBOL} premium</p>
              </div>
              <div className="text-right">
                <p className="sky-mono text-lg font-semibold" style={{ color: "var(--lime)" }}>
                  {attoToToken(pkg.coverageAtto, 0)} {TOKEN_SYMBOL}
                </p>
                <p className="text-xs text-sky-muted">max payout</p>
              </div>
            </div>
          ) : (
          ...
          <div className="mt-4 grid grid-cols-3 gap-2 border-t border-sky-border pt-3">
            {PAYOUT_TIERS.map((t) => (
              <div key={t.window} className="text-center">
                <p className="sky-mono text-sm font-bold" style={{ color: "var(--cyan)" }}>{t.pct}</p>
                <p className="mt-0.5 text-[10px] leading-tight text-sky-muted">{t.window}</p>
              </div>
            ))}
          </div>
        </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-sky-muted">{label}</span>
      {children}
    </label>
  );
}
