...
export function LiquidityVault() {
  ...
  const sharePrice =
    stats.totalShares === 0n ? 0n : (stats.totalAssets * 10n ** 18n) / stats.totalShares;
  ...
  return (
    <section className="glass sky-card rounded-3xl p-6">
      <header className="mb-5 flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold tracking-tight text-sky-text">Underwriting Vault</h2>
          <p className="text-sm text-sky-muted">Provide liquidity, earn premium yield.</p>
        </div>
      </header>

      {/* Position summary. */}
      <div className="mb-4 grid grid-cols-2 gap-3">
        <Metric label="Your Position" value={`${attoToToken(lpRedeemable, 2)} ${TOKEN_SYMBOL}`} accent="var(--cyan)" />
        <Metric
          label="Yield Earned"
      ...
      {/* Mode toggle. */}
      <div className="mb-3 grid grid-cols-2 gap-1 rounded-xl border border-sky-border bg-sky-panel-2 p-1">
        {(["deposit", "withdraw"] as Mode[]).map((m) => (
          <button
            ...
            className={`rounded-lg px-3 py-2 text-sm font-semibold capitalize transition-all duration-200 ${
              mode === m ? "bg-sky-cyan-soft text-sky-cyan" : "text-sky-muted hover:text-sky-text"
            }`}
          >
            {m}
          </button>
      ...
      <div className="space-y-3">
        ...
        <p className="sky-mono flex justify-between text-xs text-sky-muted">
        ...
        {claimable > 0n && (
          <div className="flex items-center justify-between rounded-xl border border-sky-border bg-sky-cyan-soft px-4 py-3">
            <div>
              <p className="text-xs uppercase tracking-wide text-sky-muted">Claimable</p>
...
function Metric({
  ...
  return (
    <div className="rounded-2xl border border-sky-border bg-sky-panel-2 px-4 py-3">
      <p className="flex items-center gap-1 text-[10px] font-medium uppercase tracking-wide text-sky-muted">
