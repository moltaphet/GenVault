...
import { ATTO, BASE_COVERAGE_ATTO, isSkyShieldConfigured } from "@/lib/skyshield/config";
import { payoutBpsForDelay, quotePremiumAtto, simulateRiskBps } from "@/lib/skyshield/pricing";
...
export interface LivePolicy extends Policy {
  ...
  /** True for pre-seeded, already-settled rows that belong only in the ledger. */
  historical?: boolean;
}

interface SkyShieldData {
  configured: boolean;
  /** Address whose policies/portfolio the dashboard is showing. */
  account: string;
  stats: PoolStats;
  policies: LivePolicy[];
  lpShares: bigint;
...
/** Build an already-settled boarding pass for the historical ledger. */
function makeSettledPolicy(
  policyId: number,
  flightCode: string,
  passenger: string,
  coverageAtto: bigint,
  delayMinutes: number,
  cancelled: boolean,
  resolvedAgoSec: number,
): LivePolicy {
  const bps = payoutBpsForDelay(delayMinutes, cancelled);
  const payout = (coverageAtto * BigInt(bps)) / 10_000n;
  const departureTs = nowSec() - resolvedAgoSec;
  const riskBps = simulateRiskBps(flightCode, departureTs);
  return {
    policyId,
    passenger,
    flightCode,
    departureTs,
    premiumPaid: quotePremiumAtto(coverageAtto, BigInt(riskBps)),
    maxPayout: coverageAtto,
    payoutAmount: payout,
    status: payout > 0n ? "RESOLVED" : "EXPIRED",
    riskBps,
    delayMinutes,
    createdAt: departureTs - 86_400,
    resolvedAt: departureTs + delayMinutes * 60,
    liveStatus: payout > 0n ? "RESOLVED" : "EXPIRED",
    historical: true,
  };
}
...
export function SkyShieldDataProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Pre-settled history so the ledger and portfolio are rich on first load.
    const history: LivePolicy[] = [
      makeSettledPolicy(940, "AF447", passenger, BASE_COVERAGE_ATTO, 0, true, 3_600),
      makeSettledPolicy(941, "LH118", passenger, 2_500n * ATTO, 150, false, 7_200),
      makeSettledPolicy(942, "UA022", passenger, BASE_COVERAGE_ATTO, 20, false, 10_800),
      makeSettledPolicy(943, "QF009", passenger, 5_000n * ATTO, 75, false, 86_400),
    ];

    setPolicies([...active, ...history]);
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      setPolicies((prev) => {
        let mutated = false;
        const poolDelta = { assets: 0n, locked: 0n, payouts: 0n, yield: 0n };
        let claimDelta = 0n;
